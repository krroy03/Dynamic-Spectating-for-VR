...
public class MazeGenerator : MonoBehaviour {
	...
	void StartGame() {
		if (GameManager.Instance.VR && PhotonNetwork.connectedAndReady) {
			GenerateMaze ();
			this.transform.Rotate (90.0f, 0.0f, 0.0f);
			PlacePlayerInEmptyCell ();
		}
	}

	void PlacePlayerInEmptyCell() {
		...
		for (int i = 0; i <= Maze.GetUpperBound (0); i++) {
			for (int j = 0; j <= Maze.GetUpperBound (1); j++) {
				if (Maze [i, j] == 0) {
					...
					Vector3 temp = new Vector3(ptype.transform.position.x, cubeSize / 2.0f, 0);
					GameObject.Destroy (ptype);

					Camera.main.transform.parent.transform.position = temp;
					return;
				}
